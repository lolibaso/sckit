...
struct MTState {
  ...
  //size_t index = SIZE;  /* need -std=c++11 */
  size_t index;
};
...
void mt_prng::seed(uint32_t value)
{
  /*
...
char* mt_prng::rand_string(char* str, int len, const char* chars_set)
{
    int set_len = strlen(chars_set);

	int i;
    for(i=0; i<len; i++)
    {
        str[i] = chars_set[rand_uint32()%set_len];
    }
    str[i] = 0x0;
...
int main(int argc, char* argv[])
{
    ...
    for(int i=0; i<10; i++)
    {
        printf("%10u, %s\n", sc::mt_prng::rand_uint32(), sc::mt_prng::rand_string(s, 64));
    }
    return 0;
}
#endif
