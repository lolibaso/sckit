...
/* -------------------------------------------------------------------------- */
static void sha256_init(sha256_context ctx[1])
{
...
/* -------------------------------------------------------------------------- */
static void sha256_hash(sha256_context *ctx, uint8_t *dat, uint32_t sz)
{
...
/* -------------------------------------------------------------------------- */
static void sha256_done(sha256_context *ctx, uint8_t *buf)
{
...
void 
sha256::hash(unsigned char* out_buffer, const unsigned char* in_buffer, int in_buffer_len)
{
    sha256_context ctx;

    sha256_init(&ctx);
	sha256_hash(&ctx, (uint8_t*)in_buffer, in_buffer_len);
    sha256_done(&ctx, (unsigned char *)out_buffer);

    out_buffer[32] = '\0';
}

char* 
sha256::hashstr(char* out, const unsigned char* in, int in_len)
{
    unsigned char tmp[32]= {0};

    hash(tmp, in, in_len);

    str_util::hex2string(tmp, sizeof(tmp), out);
...
int main(int argc, char *argv[])
{
    ...
    for(i=0; i < 1000000; i++ )
    {
        sc::SHA256_HashStr(out, (const unsigned char*)argv[1], strlen(argv[1]));
    }
